(()=>{
const VERSION='20260823-photo-render-v2';
const photos=[
 {src:'/images/paragon-pink.svg',alt:"Victory at Paragon '24",title:"Paragon '24.",text:'The chapter we prayed, studied and worked our way through.'},
 {src:'/images/portrait-current.svg',alt:'Victory, still becoming',title:'And here I am.',text:'Still becoming. Still grateful. Still carrying every piece.'},
...
async function run(){
 ...
 if(hero)hero.src='/images/paragon-pink.svg?v='+VERSION;
 const grid=document.querySelector('.photo-grid');if(!grid)return;
 const loaded=[];
 for(const p of photos){
   try{
     const data=await load(p.src);
     await new Promise((resolve,reject)=>{const test=new Image();test.onload=resolve;test.onerror=reject;test.src=data});
     loaded.push({...p,data});
   }catch(e){console.warn('photo skipped',p.src,e)}
 }
 if(!loaded.length)return;
 grid.className='photo-slideshow';
 grid.innerHTML='<div class="slide-stage" aria-live="polite"></div><button class="slide-nav prev" type="button" aria-label="Previous photo">‹</button><button class="slide-nav next" type="button" aria-label="Next photo">›</button><div class="slide-dots" role="tablist"></div><div class="slide-caption"></div>';
 const stage=grid.querySelector('.slide-stage'),caption=grid.querySelector('.slide-caption'),dots=grid.querySelector('.slide-dots');
 let current=0,timer;
 ...
 dots.innerHTML=loaded.map((_,i)=>'<button type="button" aria-label="Go to photo '+(i+1)+'"></button>').join('');
 dots.querySelectorAll('button').forEach((b,i)=>b.onclick=()=>{show(i);restart()});
 grid.querySelector('.prev').onclick=()=>{show(current-1);restart()};
 grid.querySelector('.next').onclick=()=>{show(current+1);restart()};
 show(0);restart();
}
setTimeout(run,300);
})();
