(()=>{
const VERSION='20260823-photo-final-3';
const photos=[
...
async function load(src){
 if(src.endsWith('.jpg')||src.endsWith('.jpeg')||src.endsWith('.png')||src.endsWith('.webp')){
   const r=await fetch(src+'?v='+VERSION,{cache:'no-store'});if(!r.ok)throw Error(src);await r.blob();return src+'?v='+VERSION;
 }
 ...
 const text=new TextDecoder().decode(bytes).replace(/\s/g,'');atob(text);
